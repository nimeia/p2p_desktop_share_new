...
bool ServiceHost::Start(const ServiceConfig& cfg) {
  ...
  try {
    ...
    auto router = std::make_shared<HttpRouter>(cfg.wwwRoot, cfg.adminRoot, impl_->hub);
    const auto ep = boost::asio::ip::tcp::endpoint{addr, cfg.port};

    impl_->listener = std::make_unique<Listener>(impl_->ioc, impl_->hub, router, ep);
    impl_->listener->Run();
...
std::string ServiceHost::BuildHostUrl(const std::string& hostIp,
  ...
  return "http://" + hostIp + ":" + std::to_string(impl_->cfg.port) +
...
std::string ServiceHost::BuildViewerUrl(const std::string& hostIp, const std::string& roomId) const {
  return "http://" + hostIp + ":" + std::to_string(impl_->cfg.port) + "/view?room=" + roomId;
}
